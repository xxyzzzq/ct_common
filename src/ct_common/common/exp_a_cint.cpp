...
Exp_A_CInt::Exp_A_CInt(void) : Exp_A_Atom(), value_(0) {}
...
void Exp_A_CInt::set_value(int value) { this->value_ = value; }

void Exp_A_CInt::set_value(double value) { this->value_ = (int)value; }

EvalType_Double Exp_A_CInt::EvaluateDouble_Impl(
    const std::vector<std::shared_ptr<ParamSpec> > &param_specs,
    const Assignment &assignment) const {
  return EvalType_Double(this->value_, true);
}

EvalType_Int Exp_A_CInt::EvaluateInt_Impl(
    const std::vector<std::shared_ptr<ParamSpec> > &param_specs,
    const Assignment &assignment) const {
...
std::string Exp_A_CInt::class_name(void) { return "Exp_A_CInt"; }
