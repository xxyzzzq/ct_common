...
Constraint_A_LT &Constraint_A_LT::operator = (
    const Constraint_A_LT &right) {
...
std::string Constraint_A_LT::class_name(void) {
  return "Constraint_A_LT";
}
...
bool Constraint_A_LT::evaluate_func_double(double loprd_val, double roprd_val) const {
  return ct::common::utils::cmp_lt(loprd_val, roprd_val, this->precision_);
}

std::string Constraint_A_LT::get_op_token(void) const {
  return ">=";
}
