...
EvalType_Bool Constraint_L_IVLD::Evaluate( const std::vector<std::shared_ptr<ParamSpec> > &param_specs,
    ...
    for (std::size_t i = 0; i < param_specs[this->pid_]->auto_value_specs().size(); ++i) {
      ...
      if (cond) {
        ...
        if (cond_value.is_valid_ && cond_value.value_) {
          ...
          std::shared_ptr<EvalType> result = this->EvaluateAutoCaseExp(param_specs, assignment, param_specs[this->pid_]->auto_value_specs()[i].second);
          if (result->is_valid_) {
...
std::shared_ptr<EvalType> Constraint_L_IVLD::EvaluateAutoCaseExp(
  ...
  if (std::dynamic_pointer_cast<Constraint>(exp)) {
    std::shared_ptr<Constraint> c_exp = std::dynamic_pointer_cast<Constraint>(exp);
    return std::shared_ptr<EvalType>(new EvalType_Bool(c_exp->Evaluate(param_specs, assignment)));
    ...
    if (c_exp->get_type() == EAT_INT) {
      return std::shared_ptr<EvalType>(new EvalType_Int(c_exp->EvaluateInt(param_specs, assignment)));
    } else if (c_exp->get_type() == EAT_DOUBLE) {
      return std::shared_ptr<EvalType>(new EvalType_Double(c_exp->EvaluateDouble(param_specs, assignment)));
    } else {
    ...
    return std::shared_ptr<EvalType>(new EvalType_String(c_exp->Evaluate(param_specs, assignment)));
  } else {
...
void Constraint_L_IVLD::dump(std::ostream &os, const std::vector<std::shared_ptr<ParamSpec> > &param_specs) const {
  os << "#";
  os << "(";
