...
Constraint_L_IVLD &Constraint_L_IVLD::operator=(
    const Constraint_L_IVLD &right) {
...
std::string Constraint_L_IVLD::class_name(void) { return "Constraint_L_IVLD"; }

EvalType_Bool Constraint_L_IVLD::Evaluate(
  ...
  // FIXME: need to reconsider the logics here, typically auto parameters should
  // not be invalidated
  if (param_specs[this->pid_]->is_auto()) {
    for (std::size_t i = 0;
         i < param_specs[this->pid_]->auto_value_specs().size(); ++i) {
      std::shared_ptr<Constraint> cond = std::dynamic_pointer_cast<Constraint>(
          param_specs[this->pid_]->auto_value_specs()[i].first);
      if (cond) {
        ...
        if (cond_value.is_valid_ && cond_value.value_) {
          ...
          std::shared_ptr<EvalType> result = this->EvaluateAutoCaseExp(
              param_specs, assignment,
              param_specs[this->pid_]->auto_value_specs()[i].second);
          if (result->is_valid_) {
  ...
  return EvalType_Bool(
      param_specs[this->pid_]->is_vid_invalid(assignment.GetValue(this->pid_)),
      true);
}

std::shared_ptr<EvalType> Constraint_L_IVLD::EvaluateAutoCaseExp(
    const std::vector<std::shared_ptr<ParamSpec> > &param_specs,
    const Assignment &assignment, const std::shared_ptr<TreeNode> &exp) const {
  if (std::dynamic_pointer_cast<Constraint>(exp)) {
    std::shared_ptr<Constraint> c_exp =
        std::dynamic_pointer_cast<Constraint>(exp);
    return std::shared_ptr<EvalType>(
        new EvalType_Bool(c_exp->Evaluate(param_specs, assignment)));
  } else if (std::dynamic_pointer_cast<Exp_A>(exp)) {
    std::shared_ptr<Exp_A> c_exp = std::dynamic_pointer_cast<Exp_A>(exp);
    if (c_exp->get_type() == EAT_INT) {
      return std::shared_ptr<EvalType>(
          new EvalType_Int(c_exp->EvaluateInt(param_specs, assignment)));
    } else if (c_exp->get_type() == EAT_DOUBLE) {
      return std::shared_ptr<EvalType>(
          new EvalType_Double(c_exp->EvaluateDouble(param_specs, assignment)));
    } else {
      CT_EXCEPTION("unrecognized arithmetic expression type");
    }
  } else if (std::dynamic_pointer_cast<Exp_S>(exp)) {
    std::shared_ptr<Exp_S> c_exp = std::dynamic_pointer_cast<Exp_S>(exp);
    return std::shared_ptr<EvalType>(
        new EvalType_String(c_exp->Evaluate(param_specs, assignment)));
  } else {
