...
Exp_A_Binary::Exp_A_Binary(void) : Exp_A() { this->oprds_.resize(2); }

Exp_A_Binary::Exp_A_Binary(const Exp_A_Binary &from) : Exp_A(from) {}
...
std::string Exp_A_Binary::class_name(void) { return "Exp_A_Binary"; }

EvalType_Double Exp_A_Binary::EvaluateDouble_Impl(
  ...
  if (this->get_loprd()->get_type() == EAT_INT &&
    ...
    GET_EXP_VAL(EvalType_Double, val_l, this->get_loprd(), param_specs,
                assignment);
    GET_EXP_VAL(EvalType_Double, val_r, this->get_roprd(), param_specs,
                assignment);
    tmp_return.is_valid_ = val_l.is_valid_ && val_r.is_valid_;
    if (tmp_return.is_valid_) {
...
EvalType_Int Exp_A_Binary::EvaluateInt_Impl(
    const std::vector<std::shared_ptr<ParamSpec> > &param_specs,
    const Assignment &assignment) const {
  ...
  if (this->get_loprd()->get_type() == EAT_INT &&
    ...
    if (tmp_return.is_valid_) {
      tmp_return.value_ =
          (int)this->evaluate_double(val_l.value_, val_r.value_);
    }
  } else {
    CT_EXCEPTION("cannot evaluate this expression");
  }
