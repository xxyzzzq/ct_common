...
Exp_A_CDouble::Exp_A_CDouble(void)
    : Exp_A_Term(), value_(0), str_value_() {
}
...
EvalType_Double Exp_A_CDouble::EvaluateDouble_Impl(const std::vector<boost::shared_ptr<ParamSpec> > &param_specs,
                                          const Assignment &assignment) const {
  return EvalType_Double(this->value_, true);
}

EvalType_Int Exp_A_CDouble::EvaluateInt_Impl(const std::vector<boost::shared_ptr<ParamSpec> > &param_specs,
                                    const Assignment &assignment) const {
...
void Exp_A_CDouble::dump(std::ostream &os, const std::vector<boost::shared_ptr<ParamSpec> > &param_specs) const {
