...
Constraint_L_Or &Constraint_L_Or::operator = (
    const Constraint_L_Or &right) {
...
std::string Constraint_L_Or::class_name(void) {
  return "Constraint_L_Or";
}

EvalType_Bool Constraint_L_Or::evaluate_func(EvalType_Bool val_l, EvalType_Bool val_r) const {
  ...
  if (val_l.is_valid_ && val_r.is_valid_) {
    ...
    tmp_return.is_valid_ = (val_l.is_valid_ && val_l.value_) || (val_r.is_valid_ && val_r.value_);
    tmp_return.value_ = tmp_return.is_valid_;
  }
  return tmp_return;
}

std::string Constraint_L_Or::get_op_token(void) const {
  return "||";
}
