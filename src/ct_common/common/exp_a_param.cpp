...
Exp_A_Param::Exp_A_Param(void) : Exp_A_Atom(), pid_(PID_BOUND) {}
...
EvalType_Double Exp_A_Param::EvaluateDouble_Impl(
  ...
  if (param_specs[this->pid_]->is_auto()) {
    ...
    for (std::size_t i = 0;
         i < param_specs[this->pid_]->get_auto_value_specs().size(); ++i) {
    ...
    // condition for parameter ")+param_specs[this->pid_]->get_param_name());
    return tmp_return;
  }
  ...
  if (tmp_return.is_valid_) {
    if (TYPE_CHECK(param_specs[this->pid_].get(), ParamSpec_Int *)) {
      tmp_return.value_ =
          dynamic_cast<ParamSpec_Int *>(param_specs[this->pid_].get())
              ->get_int_values()[vid];
    } else if (TYPE_CHECK(param_specs[this->pid_].get(), ParamSpec_Double *)) {
      tmp_return.value_ =
          dynamic_cast<ParamSpec_Double *>(param_specs[this->pid_].get())
...
EvalType_Int Exp_A_Param::EvaluateInt_Impl(
    const std::vector<std::shared_ptr<ParamSpec> > &param_specs,
    const Assignment &assignment) const {
  ...
  if (tmp_return.is_valid_) {
    if (TYPE_CHECK(param_specs[this->pid_].get(), ParamSpec_Int *)) {
      ...
      tmp_return.value_ =
          (int)dynamic_cast<ParamSpec_Double *>(param_specs[this->pid_].get())
              ->get_double_values()[vid];
    } else {
      CT_EXCEPTION(
          "cannot evaluate numerical value of a non-numerical parameter");
...
std::string Exp_A_Param::class_name(void) { return "Exp_A_Param"; }
